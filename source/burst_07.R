...
sourceCpp(here::here("source", "burst_07.cpp"))
...
burst <- function(seed) {
  ...
  iter <- 1000000
  prefix <- "burst"
  fname <- paste0("burst_07_", seed, ".png")
  transparency <- "aa"
  adjust <- function(x) {x}
  brd <- 0
  filter_y <- c(-layers, layers)
  filter_x <- c(-layers, layers)
  ...
  pal <- (colorRampPalette(sample_shades(10)))(n=256)
  pal <- adjust(pal)
