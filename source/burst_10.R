...
sourceCpp(here::here("source", "burst_10.cpp"))
...
burst <- function(seed) {
  ...
  shades <- paletteer::paletteer_c("grDevices::TealRose", 32)
  shades <- c(shades[1:10], shades[23:32])

  # fixed
  iter <- 5000000
  prefix <- "burst"
  fname <- paste0("burst_10_", seed, ".png")
  fname2 <- paste0("burst_10_", seed, ".jpg")
  transparency <- "20"
  adjust <- function(x) {x}
  brd <- 0
