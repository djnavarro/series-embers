...
sourceCpp(here::here("source", "burst_12.cpp"))
...
burst <- function(seed) {
  ...
  iter <- 200000
  prefix <- "burst"
  fname <- paste0("burst_12_", seed, ".png")
  fname2 <- paste0("burst_12_", seed, ".jpg")
  transparency <- "ff"
  adjust <- function(x) {x}
  brd <- 0
  ...
  cb$add_circles(x=df[,1], y = df[,2], r=15, fill = col, colour = NA)
  cb$write_png(fpath)
