...
burst <- function(seed) {
  ...
  cat(fname, "\n")
  cb <- cairobasic::CairoBasic$new(width = x_px/4, height = y_px/4, bg = bg, antialias = TRUE)
  cb$add_circles(x=df$x/4, y = df$y/4, r = radius/4, fill = col, colour = NA)
  cb$write_png(fpath)
